...
def prune_sae(sae, tokens):
    for name, param in sae.named_parameters():
        if 'W' in name:
            _, cache = gpt2.run_with_cache_with_saes(tokens, saes=[sae])

            if 'enc' in name:
                X = cache[f"{sae.cfg.hook_name}.hook_sae_acts_pre"][0].norm(p=2, dim=0)
                # print(name, param.shape, X.shape)
                sae.W_enc = t.nn.Parameter(wanda(param, X, sparse_ratio=0.5))

            else:
                X = cache[f"{sae.cfg.hook_name}.hook_sae_recons"][0].norm(p=2, dim=0)
                # print(name, param.shape, X.shape)        
                sae.W_dec = t.nn.Parameter(wanda(param, X, sparse_ratio=0.5))
...
if __name__ == "__main__":
    ...
    for batch in tqdm(openwebtext):
        gpt2_sae = prune_sae(gpt2_sae, batch)
    
    t.save(gpt2_sae.state_dict(), 'pruned_gpt2_sae_wanda.pth')
